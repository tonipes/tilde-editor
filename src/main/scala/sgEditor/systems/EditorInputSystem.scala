...
import sgEngine.util.{Quaternion, Vec3}
...
class EditorInputSystem() extends EntitySystem{
  ...
  private def basicCameraMovement(cameraSpat: SpatialComponent, delta: Float): Unit = {
    ...
    if(Input.isPressed(GLFW_KEY_Q)){
      cameraSpat.orientation *= Quaternion.fromAxisAngle(Vec3(0,1,0),rotationSpeed)
    }
    if(Input.isPressed(GLFW_KEY_E)){
      cameraSpat.orientation *= Quaternion.fromAxisAngle(Vec3(0,1,0),-rotationSpeed)
    }
    if(Input.isPressed(GLFW_KEY_O)){
      cameraSpat.orientation *= Quaternion.fromAxisAngle(Vec3(1,0,0),rotationSpeed)
    }
