...
class RenderSystem extends EntitySystem{
  ...
  lazy val framebuffer = new tilde.graphics.Framebuffer(1024,1024,"diffuse","normal")
  lazy val screenPlane = Factory.createScreenMesh()
  val grid = Factory.createGridMesh(8,8,16,16)
  ...
  override def systemEnd(): Unit = { // Render
    ...
    val projectionBuffer = BufferCreator.createFloatBuffer(ResourceManager.getProjection)

    framebuffer.bindFramebuffer()
    glDrawBuffers(BufferCreator.createIntBuffer(framebuffer.attachments))
  ...
  private def renderGrid(viewBuffer: FloatBuffer, projectionBuffer: FloatBuffer) = {
    val spat = SpatialComponent(Vec3(0,-1,0),Quaternion(0,0,0,1),Vec3(1,1,1))
  ...
  private def getFloatBuffer(position: Vec3, orientation: Quaternion, scaleVec: Vec3): FloatBuffer ={
    ...
    BufferCreator.createFloatBuffer( mul.transpose )
  }
